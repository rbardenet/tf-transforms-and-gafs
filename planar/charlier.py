...
def cmp(llambda, values, normalize=True):
    """Evaluate unnormalized pdf of Conway-Maxwell-Poisson of parameter llambda"""
    if not np.all(np.ceil(values)==values):
        raise("IncorrectType")
    if not np.shape(values):
        values = np.array(values)
    res = np.zeros(values.shape, dtype='float')
    ind = (values>=0)
    res[ind] = np.exp(values[ind]*np.log(llambda) - .5*sps.gammaln(values[ind]+1) - llambda)
    if normalize:
        res /= np.max(res)  # non-standard normalization, but it preserves zeros
    return res
